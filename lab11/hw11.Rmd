...
gapminder %>% 
  select(country, year, pop) %>% 
  filter(country == "India", year<=2025, year>=1925) %>% 
  ggplot(aes(x=year, y=pop))+
  geom_line(color= "red")+
  labs(title = "India's population growth from 1925 to 2025", x="year", y="population")
```
...
gapminder %>% 
  ...
  labs(title = "life expectancy for all countries in 1925 and 2025", x="life expectancy", y="density", fill= "year")
```
...
gapminder %>% 
  ...
  labs(title= "relationship between per capita GDP and life expectancy", x= "GDP", y="life expectancy")
```
...
gdp_country <- gapminder %>% 
  ...
  pivot_wider(names_from = year,
              values_from = gdp,
              names_prefix = "year_") %>% 
  mutate(gdp_grow = year_2025 - year_1975) %>% 
  arrange(desc(gdp_grow)) %>% 
  slice_head(n=5)
gdp_country
```
...
gapminder %>% 
  ...
  geom_line()+
  geom_point()+
  labs(title = "capita GDP growth in top 5 countries in the past 50 years", x= "year", y= "GDP per capita", color= "country")
```

10. Do one analysis of your choice that includes a table and plot as outputs.

Average life expectancy per country in 2025 for the top 5 countries with highest capita GDP growth in the last 50 years
...
gapminder %>% 
  filter(country == "Singapore"|
        ...
        country == "Norway",
         year== 2025) %>% 
  group_by(country) %>% 
  summarise(mean_life= mean(lex, na.rm=T),
            min_life= min(lex, na.rm= T),
            max_life= max(lex, na.rm = T),
            median_life= median(lex, na.rm = T))
```
...
gapminder %>% 
  filter(country == "Singapore"|
         ...
         year== 2025) %>% 
  group_by(country) %>% 
  summarise(mean_life= mean(lex)) %>% 
  ggplot(aes(x= country, y=mean_life)) +
  geom_col(aes(fill = country))+
  labs(title = "avg life expectancy in 2025 for top 5 GDP growth countries",
