...
gap %>% 
  ...
  labs(title = "distribution of life exectancy")
```
...
gap %>% 
  filter(year==1935) %>% 
  ggplot(aes(x=lex))+
  geom_histogram(aes(y=after_stat(density)), colour = "black", fill = "turquoise4")+
...
gap %>% 
  ...
  ggplot(aes(x=lex, colour = year, fill = year))+
  geom_density(alpha=0.4)+
  labs(title = "distributon of life expectancy, 1930 and 1950",
...
income_cat2000 %>% 
  ...
  geom_bar()+
  labs(title = "countries in the WB income categories",
       x="income categories",
       y=NULL)+
