...
```{r}
gabon <- read.csv("C:/Users/jk_ic/OneDrive/Desktop/BIS15L_W26_jrkwok/lab6/data/IvindoData_DryadVersion.csv")
```
...
gabon_new %>% 
  select(land_use) %>% 
  ggplot(data = gabon_new,
         mapping= aes(x=land_use))+
  geom_bar()+
  labs(title = "number of transects for each land use type", x= "land use type", y= "number of transects")
```
...
gabon_new %>% 
  ...
  geom_boxplot()+
  labs(title = "vegetation richness among land use types", x= "land use types", y= "vegetation richness")
```
...
gabon_new %>% 
  ...
  geom_boxplot()+
  labs(title = "all species diverity for each land use type", x= "land use type", y= "diversity for all species")
```
...
gabon_new %>% 
  ...
  labs(title= "vegetation richness vs mammal species richness", x= "mammal richness", y="vegetation richness")
```
...
gabon_new %>% 
  ...
  filter(veg_rich>15 | veg_dbh %in% c(50,60))
```
