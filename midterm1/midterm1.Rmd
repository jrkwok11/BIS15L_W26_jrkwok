...
\
\
\ https://github.com/jrkwok11/BIS15L_W26_jrkwok 
...
\
```{r}
anolis <- read.csv("~/Desktop/BIS15L_W26_jrkwok/midterm1/data/anolis_dat.csv") 
```
...
anolis %>% 
  ...
  labs(title = "Number of species on each island", x="Island names", y="Number of species")
```
...
anolis %>% 
  ...
  labs(title = "Number of species in each habitat type", x= "habitat type", y="number of species")
```
...
anolis %>% 
  ...
  labs(title = "Variation of hindlimb length (mm) in each habitat type", x="habitat type", y="length of hindlimbs (mm)")
```
...
```{r}
ratio_of_tail_to_body <- anolis$tail_length_mm/ anolis$body_length_mm
```

```{r}
anolis_new<- tibble(ratio_of_hindlimb_to_body, ratio_of_tail_to_body, anolis)
```
...
\ This plot shows that hindlimb to body length ratios for species in Grass-bush habitats are greater than the ratio in Trunk-crown habitats. This is a different representation than from question 10's plot where it shows that Grass-bush individuals have shorter hindlimbs on average than Trunk-crown. This graph shows that the ratio of hindlimbs to body is higher for individuals living in Grass-bush habitats than in Trunk-crown habitats, meaning more of the body is made of their hindlimbs in grass-bush habitats.
\
\
\
\
```{r}
anolis_new %>% 
  select(ratio_of_hindlimb_to_body, habitat) %>% 
  ggplot(mapping = aes(x=habitat, y=ratio_of_hindlimb_to_body))+
  geom_boxplot(mapping = aes(fill = habitat))+
  labs(title = "Ratio of hindlimbs to body length in each habitat type", x="habitat type", y="ratio of hindlimbs to body length")
```
...
\ I noticed that lizards in the Grass-bush habitat have typically longer tails compared to their body length. They have some of the smallest body lengths with the longest tail lengths. Their measurements are mostly off the line of best fit
\
\
\
\
...
anolis %>% 
  ...
  summarise(mean_toepad_trunkground = mean(toepad_lamellae_count))
```
```{r}
anolis %>% 
  select(habitat, toepad_lamellae_count) %>% 
  filter(habitat == "Grass-bush") %>% 
  summarise(mean_toepad_grassbush = mean(toepad_lamellae_count))
```
```{r}
anolis %>% 
  select(habitat, toepad_lamellae_count) %>% 
  filter(habitat == "Trunk-crown") %>% 
  summarise(mean_toepad_trunkcrown = mean(toepad_lamellae_count))
```
```{r}
anolis %>% 
  select(habitat, toepad_lamellae_count) %>% 
  filter(habitat == "Twig") %>% 
  summarise(mean_toepad_twig = mean(toepad_lamellae_count))
```
...
anolis %>% 
  ...
  labs(title = "Number of toepad lamellae of trunk-crown species across all islands", x="island name", y="number of toepad lamellae of trunk-crown species")
```
