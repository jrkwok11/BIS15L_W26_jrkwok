...
fisheries_clean %>% 
  select(common_name) %>% 
  summarise(number_species_common= n_distinct(common_name))
```
```{r}
fisheries_clean %>% 
  select(taxonomic_code) %>% 
  summarise(number_species_common= n_distinct(taxonomic_code))
```
...
fisheries_clean %>% 
  ...
  summarise(caught_species= sum(catch)) %>% 
  arrange(desc(caught_species)) %>% 
  slice_head(n=10)
```
...
```{r}
fisheries_clean %>% 
  select(catch, common_name, country, period) %>% 
  filter(common_name== "Alaska pollock(=Walleye poll.)",
         period=="2023") %>% 
  group_by(country) %>% 
  summarise(catch_country= sum(catch)) %>% 
  arrange(desc(catch_country))
```
