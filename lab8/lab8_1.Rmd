...
```{r}
elephants %>% 
  select(estimated_age_years, shoulder_height_in_cm, tusk_length_in_cm, tusk_circumference_in_cm) %>% 
...
elephants %>% 
  ...
  summarise(mean_est_age_yrs = mean(estimated_age_years, na.rm=T),
            n=n(),
            .groups = 'keep') %>% 
  arrange(sex)
```
...
elephants %>% 
  ...
  group_by(sex) %>% 
  summarise(age_mean=mean(estimated_age_years, na.rm=T),
            age_median= median(estimated_age_years, na.rm = T),
...
msleep %>% 
  group_by(order) %>% 
  summarise(mean(brainwt, na.rm= T))
```
